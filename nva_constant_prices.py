...
for district_sheet in wb.worksheets:
    ...
    district_sheet['D115'].value = "Net Value Added ( at Constant Prices )"
    # copy style
    fontStyle = Font(size = "15", bold = True)
    alignment = Alignment(horizontal='center', vertical='center')
    #fill = PatternFill("solid", fgColor="FFFFFF")

    district_sheet['D115'].font = fontStyle
    district_sheet["D115"].alignment = alignment
    #district_sheet["D1"].fill = fill

    district_sheet['D116'].font = fontStyle
    district_sheet['D116'].alignment = alignment
...
for district_sheet in wb.worksheets:
    ...
    if district_sheet.title == "Charkhi Dadri": 
        ws = rawbook["2017-18"]
        district_sheet.cell(row=117, column=3).value = "Charkhi Dadri"
        district_sheet.cell(row=118, column=3).value = "2017-18"
        for row in range(119,152):
    ...
    for year_sheet in rawbook.worksheets:
        ...
        district_sheet.cell(row=117, column=j).value = district_sheet.title
        # assign year name to corresponding column in district sheet
        district_sheet.cell(row=118, column=j).value = year_sheet.title
