...
for district_sheet in wb.worksheets:
    ...
    district_sheet['D39'].value = "Gross Value Added (at Constant Prices)"
    # copy style
    fontStyle = Font(size = "15", bold = True)
    alignment = Alignment(horizontal='center', vertical='center')
    #fill = PatternFill("solid", fgColor="FFFFFF")

    district_sheet['D39'].font = fontStyle
    district_sheet["D39"].alignment = alignment
    #district_sheet["D1"].fill = fill

    district_sheet['D40'].font = fontStyle
    district_sheet['D40'].alignment = alignment
...
for district_sheet in wb.worksheets:
    ...
    if district_sheet.title == "Charkhi Dadri": 
        ws = rawbook["2017-18"]
        district_sheet.cell(row=41, column=3).value = "Charkhi Dadri"
        district_sheet.cell(row=42, column=3).value = "2017-18"
        for row in range(43,76):
    ...
    for year_sheet in rawbook.worksheets:
        ...
        district_sheet.cell(row=41, column=j).value = district_sheet.title
        # assign year name to corresponding column in district sheet
        district_sheet.cell(row=42, column=j).value = year_sheet.title
