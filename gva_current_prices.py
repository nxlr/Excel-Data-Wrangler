...
for district_sheet in wb.worksheets:
    ...
    district_sheet['D1'].value = "Gross Value Added ( at Current Prices )"
    # copy style
    fontStyle = Font(size = "15", bold = True)
    alignment = Alignment(horizontal='center', vertical='center')
    #fill = PatternFill("solid", fgColor="FFFFFF")

    district_sheet['D1'].font = fontStyle
    district_sheet["D1"].alignment = alignment
    #district_sheet["D1"].fill = fill

    district_sheet['D2'].font = fontStyle
    district_sheet['D2'].alignment = alignment
...
for district_sheet in wb.worksheets:
    ...
    if district_sheet.title == "Charkhi Dadri": 
        ws = rawbook["2017-18"]
        district_sheet.cell(row=3, column=3).value = "Charkhi Dadri"
        district_sheet.cell(row=4, column=3).value = "2017-18"
    ...
    for year_sheet in rawbook.worksheets:
        ...
        district_sheet.cell(row=3, column=j).value = district_sheet.title
        # assign year name to corresponding column in district sheet
        district_sheet.cell(row=4, column=j).value = year_sheet.title
